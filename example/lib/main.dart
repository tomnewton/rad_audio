...
class _MyAppState extends State<MyApp> implements IAudioPlayer {
  ...
  void play() {
    ...
    RadAudioPrepareToPlayMsg msg = new RadAudioPrepareToPlayMsg(
        "https://traffic.megaphone.fm/GLT7778633911.mp3", pathToArtwork, "Crimetown", "Courtney");
  ...
  void playbackProgress(double pos) {
    print(pos);
    setState(() {
      double sliderPos = 300.0 * (pos / this.duration);
      this.sliderValue = sliderPos;
    });
  }
          ...
          body: new Center(
            ...
            children: <Widget>[
              new Row(
                children: <Widget>[
                  new MaterialButton(
                      onPressed: () => this.fetchDebugAlbumArt(), child: new Text("DL"), color: Colors.orange)
                ],
              ),
              ...
              new RadSlider(300.0, 7.0,
                  onDragComplete: this.dragStopped, currentPosition: this.sliderValue, backgroundColor: Colors.green),
            ],
          )),
        ),
      );
